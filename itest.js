...
// https://forum.gather.town/t/pick-a-starter-pokemon-to-follow-you-around/335
...
function movePokemon(context) {
  ...
  let location = getNewPokeLocation(
    context.player.x,
    context.player.y,
    context.player.direction
  ); //index 0 is x and 1 is y, and 2 is direction
  let state = "normal";
  let imageLocation = eval(context.player.affiliation)[location[2]];

  //check if image should alternate
  if (game.getObject(objectId) != undefined) {
    //something something this gets mad at me if this is undefined and i try to move on
...
function pokemon() {
  ...
  game.subscribeToEvent("playerMoves", (data, context) => {
    ...
    if (context.player.affiliation != "") {
      //only try and set pokmon if player has selected one
